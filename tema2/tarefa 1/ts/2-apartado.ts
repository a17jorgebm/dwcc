const botonIf:HTMLElement=document.getElementById('boton-edad-if')
const botonSwitch:HTMLElement=document.getElementById('boton-edad-switch')

botonIf.addEventListener('click',comprobarEdadIf)
botonSwitch.addEventListener('click',comprobarEdadSwitch)

function comprobarEdadIf():void{
    let cuadro:string=prompt('Introduce una edad')
    let edad:number
    if (cuadro ===null ){
        return
    }else{
...
function comprobarEdadSwitch():void{
    let cuadro:string=prompt('Introduce una edad')
    let edad:number
    if (cuadro ===null ){
        return
